...
import HourlyForecast from './components/HourlyForecast'; // 📈 New Import

function App() {
  const [data, setData] = useState(null);
  const [forecast, setForecast] = useState([]);
  const [hourly, setHourly] = useState([]); // 📈 New State for Graph
  ...
  const fetchWeather = (lat, lon, city) => {
    ...
    axios.get(weatherUrl).then((response) => {
      ...
      // 1. Get Daily Forecast (Filter for Noon)
      const dailyData = response.data.list.filter(reading => reading.dt_txt.includes("12:00:00"));
      setForecast(dailyData);
      
      // 2. Get Hourly Data (Raw List) for the Graph
      setHourly(response.data.list);
        ...
        {!loading && data && (
          <>
            ...
            <div className="right-panel">
              ...
              {/* 📈 NEW: HOURLY GRAPH (Placed before Stats) */}
              {hourly.length > 0 && (
